 ...
 */
int
model::ReadMD5Model (const char *filename, struct md5_model_t *mdl)
{
  FILE *fp;
  char buff[512];
  int version;
 ...
 */
void
model::PrepareMesh (const struct md5_mesh_t *mesh,
...
void
model::init (const char *filename, const char *animfile)
{
...
model::draw (int dt, int texID){
 ...
 /* angle += 25 * (curent_time - last_time);
  ...
  //     Animate (m.md5anim, m.animInfo, curent_time - last_time);

  //     /* Interpolate skeletons between two frames */
  //     InterpolateSkeletons (m.md5anim.skelFrames[animInfo.curr_frame],
  //       		    m.md5anim.skelFrames[animInfo.next_frame],
  //       		    m.md5anim.num_joints,
  //       		    m.animInfo.last_time * m.md5anim.frameRate,
  //       		    m.skeleton);
  //   }
  // else
  //   {
